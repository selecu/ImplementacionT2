...
namespace I94CruzRombo
{
    public class PointManager : MonoBehaviour
    {
        ...
        [ContextMenu("Set id by obj.name")]
        ...
        void Start()
        {
            ...
            lines[counter].AddComponent<v1.Lines>();
        ...
        void OnMouseOver()
        {
            GameManagerPuntos.state = 2;
            GameManagerPuntos.two = id;
        }
        ...
        void OnMouseUp()
        {
            ...
            if (GameManagerPuntos.state != 2)
            {
                GameManagerPuntos.state = 0;
                lines[counter].GetComponent<LineRenderer>().SetPosition(1, gameObject.transform.position);
            }
            if (GameManagerPuntos.state == 2)
            {
                lines[counter].GetComponent<v1.Lines>().one = GameManagerPuntos.one;
                lines[counter].GetComponent<v1.Lines>().two = GameManagerPuntos.two;
                ...
                GameManagerPuntos.state = 1;
                int[] temp = { GameManagerPuntos.one, GameManagerPuntos.two };
                GameManagerPuntos.pairLines.Add(temp);
            }
        }
    }
}
