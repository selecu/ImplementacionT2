...
namespace I2817
{
	public sealed class Board : MonoBehaviour
	{
		...
		[Space(15)]
		public UnityEvent OnAchievingTheGoal;
		...
		private async Task<bool> TryMatchAsync()
		{
			...
			while (match != null)
			{
				...
				Scoretxt.text = SendTextValue();
                FillImage();


                if (Score >= goalScore)
                {
					OnAchievingTheGoal.Invoke();
				}
		...
		private void FillImage() =>
			imageToFill.fillAmount = (float)Score / (float)goalScore;

		private string SendTextValue() =>
			$"{Score}/{goalScore}";
