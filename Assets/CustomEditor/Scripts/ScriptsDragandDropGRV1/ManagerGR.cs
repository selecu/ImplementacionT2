...
namespace v1
{
    public class ManagerGR : MonoBehaviour
    {
        public bool Correct;
        public DragHandlerGr[] drags;
       public GameObject check_integrador;
        ...
        void Start()
        {
            drags = FindObjectsOfType<DragHandlerGr>();
            ...
            if (drags.Length != 0)
            {
                foreach (var item in drags)
                {
                    ...
                    else
                    {
                        try
                        {
                            item.GetComponentInChildren<DragHandlerGr>().StartPosition();
                        }
