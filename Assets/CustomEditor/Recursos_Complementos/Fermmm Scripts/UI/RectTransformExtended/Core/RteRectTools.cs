...
{      
    ...
    /// <param name="transf"></param>
    /// <returns></returns>
    private static Vector2 GetPositionIgnoringAnchors(RectTransform transf)
    {
        Vector2 parentSize = GetParentSizeIgnoringAnchors(transf);

        return new Vector2
            (
                transf.anchoredPosition.x + parentSize.x  * RteAnchorTools.GetAnchorsPosition(transf,true).x,
                transf.anchoredPosition.y + parentSize.y * RteAnchorTools.GetAnchorsPosition(transf,true).y
            );
    }
    ...
    {
        Vector2 nonAnchoresPos = GetPositionIgnoringAnchors(transf);
        ...
        return new Vector2
        (
            nonAnchoresPos.x - (transf.rect.width * transf.pivot.x) + size.x * 0.5f,    
            nonAnchoresPos.y - (transf.rect.height * transf.pivot.y) + size.y * 0.5f
        );
    }
    ...
    public static void SetPositionIgnoringAnchors(RectTransform transf, Vector2 newPosition)
    {
        ...
        transf.anchoredPosition = newPosition - new Vector2
            (
                parentSize.x * RteAnchorTools.GetAnchorsPosition(transf,true).x,
                parentSize.y * RteAnchorTools.GetAnchorsPosition(transf,true).y
            );
    }
    ...
    public static void SetPositionIgnoringAnchorsAndPivot(RectTransform transf, Vector2 newPosition, bool centerPivot = false)
    {
        ...
        transf.anchoredPosition = newPosition - new Vector2
            ...
            + new Vector2
            (
                (transf.rect.width * transf.pivot.x)  - size.x * 0.5f,
                (transf.rect.height * transf.pivot.y) - size.y * 0.5f
            );
    }
