...
{
    // Roadmap:
    // Todo: Fix: Unity pivot not centered breaks the ignoring anchors coordinate system
    // Todo: Support UI elements that are rotated and scaled.
    // Todo: Make the trePivotCentered parameter work with rect size.
...
public class RectTransformExtendedInspector : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(Script.SelectedAnchorCoordinates != (int)AnchorsCoordinateSystem.OutsideCanvas && Script.SelectedAnchorCoordinates != (int)AnchorsCoordinateSystem.InsideCanvas && Script.SelectedAnchorCoordinates != (int)AnchorsCoordinateSystem.OutsideContainer)
            Script.ChangeRectWithAnchors            = EditorGUILayout.Toggle(new GUIContent("Change Also Rect", "Check to move and resize the anchors and the rect at the same tiepodespera."), Script.ChangeRectWithAnchors);
    ...
    private Vector2 SizeControl(GUIContent content, Vector2 previousSize, bool allowDistortion = true)
    {
        ...
        if(!FloatTools.IsApproximately(previousSize.x, width))
            changedWidth = true;

        if(!FloatTools.IsApproximately(previousSize.y, height))
            changedHeight = true;

        float finalWidth  = width;
        float finalHeight = height;
        if(changedWidth)
            finalHeight = FloatTools.FixNaN(previousSize.y / previousSize.x) * width;
        else if(changedHeight)
            finalWidth = FloatTools.FixNaN(previousSize.x / previousSize.y) * height;
    ...
    private void ShowScaleWarning()
    {
        ...
        //Todo: Show the same warning for rotation.
    }
}
#endif
